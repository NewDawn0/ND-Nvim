...
function M.win_input(window_name, callback)
    ...
    vim.api.nvim_buf_set_keymap(0, "i", "<Cr>", "<cmd>stopinsert | lua require('custom.command').get_input_win_text(" .. win .. ")<CR>", opts)
end
...
local function win_exists(window_name)
    for _, win in ipairs(api.nvim_list_wins()) do
        ...
        if api.nvim_buf_get_name(buf):match("[^/]+$") == window_name then
            return win
        end
    end
    return false
...
-- Imporved :new
cmd("New", function ()
    M.win_input("Enter new name", function (text)
        vim.cmd("new " .. text)
    end)
end, {})
