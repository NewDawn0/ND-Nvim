...
if langs.html.enabled or langs.css.enabled or langs.javascript.enabled or langs.typescript.enabled then
  cmd("LiveServer", function()
    ...
    vim.fn.jobstart("live-server", {
      on_exit = function() vim.notify "Live-Server exited" end,
    })
  end, {})
end
...
cmd("NDNvimInstallInfo", function()
  ...
  print "[INFO] Enable languages using pkgs.ndnvim.override { opts.langs.<language>.enabled = true; };"
  print ""
  ...
  for lang, settigns in pairs(vim.g.ndnvim.langs) do
    if lang ~= "all" then
      if settigns.enabled then
        print("[✓]  " .. lang)
      else
        print("[x]  " .. lang)
      end
